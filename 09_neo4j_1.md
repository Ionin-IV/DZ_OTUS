...
- summary - список отзывов со связкой с персонами и фильмами

Команды создания структуры нормализованных таблиц:
```
...
__Результат:__ и снова запрос в neo4j выглядит проще. Однако, если бы некоторые персоны данной БД не имели по несколько типов отношений (например, один и тот же человек может быть и актёром и режиссёром) к фильмам, то можно было бы сделать отдельные таблицы со списком актёров, режиссёров и т.д.

Так же, если запрашивать загруженные данные из не нормализованной таблицы, то запросы будут проще:
