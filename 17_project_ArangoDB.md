...
1. Провести сравнение СУБД ArangoDB, как графовой, с Neo4j.
2. Провести сравнение СУБД ArangoDB, как документоориентированной, с MongoDB.
...
__РЕЗУЛЬТАТ__: скокрость выполнения запроса, после построения индексов) в обеих БД многократно увеличилась и стала равна примерно 1 мс.
...
### Выводы по сравнению СУБД ArangoDB, как графовой, и Neo4j

1. Графовые данные для загрузки в ArangoDB нужно предварительно готовить, т.к. загрузка каждого узла и связи производится из отдельного набора данных. При загрузке в Neo4j данные из json можно раскладывать по узлам и связям прямо из каждой записи, что гораздо удобней.
2. Графовые данные в ArangoDB хранятся в виде таблиц с документами (узлы в коллекциях, а связи в специальных edge коллекциях), что удобно, т.к. можно обратиться к ним напрямую. Так же можно построить и граф по уже имющимся в БД таблицам с документами. Это тоже большой плюс.
3. Большой объём графовых данных в ArangoDB загружается очень быстро за доли секунд. В Neo4j большие данные загружаются с уменьшением скорости по мере увеличения уже загруженных данных. Есть предположение, что чем больше данных в БД уже есть, тем дольше ищутся уже имеющиеся данные и встраиваются в них загружаемые. Индексы немного улучшают ситуацию, но всё равно очень долго.
4. Синтаксис и сложность логики построения запросов в ArangoDB и Neo4j примерно сопоставимы.
5. В ArangoDB создаются индексы по умолчанию (primary индекс по _key на коллекциях и edge индекс по _to, _from на edge коллекциях), что сразу позволяет получить отличную производительность выполнения графовых запросов, без достроения дополнительных индексов. Если в Neo4j создать индексы, то производительность графовых запросов примерно сопоставимая.

### Выводы по сравнению СУБД ArangoDB, как документоориентированной, и MongoDB

1. Большие данные из json-файла в ArangoDB загружаются быстрее, чем в MongoDB (1000000 документов за 2 и 15 секунд, соответственно).
2. Синтаксис и сложность логики построения запросов в ArangoDB несколько проще, чем в MongoDB, т.к. код более похож на обычный SQL.
3. Скорость выполнения запросов без индексов и с построенными индексами примерно одинаковая.

### Общий вывод по ArangoDB

1. ArangoDB проявляет себя отлично и, как графовая, и как документоориентированная. Вижу хороший вариант её использования: хранить в ней документы, с возможностью дополнительно строить по ним же графы.
2. По производительности ArangoDB не уступает одним из лучшим СУБД в своей области: графовой Neo4j и документоориентированной MongoDB. Во всяком случае, в не кластерном исполнении.
